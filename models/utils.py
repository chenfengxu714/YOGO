...
def create_pt_components(blocks, in_channels, t_layer, head, ffn_expansion=2,
        ffn_groups=1, tokenizer_tau=1, proj_tau=1, width_multiplier=1, sample_method='random'):
    r = width_multiplier

    layers, concat_channels = [], 0
    for out_channels, num_blocks, token_num, point_num, pos_dim in blocks:
        out_channels = int(r * out_channels)
        if token_num is None:
            block = SharedMLP
        else:
            block = functools.partial(PTConv, point_num=point_num, token_num=token_num, pos_dim=pos_dim, t_layer=t_layer, head=head, ffn_expansion=ffn_expansion,
                                      ffn_groups=ffn_groups, tokenizer_tau=tokenizer_tau, proj_tau=proj_tau, sample_method=sample_method)
        for _ in range(num_blocks):
            layers.append(block(in_channels, out_channels))
            in_channels = out_channels
            concat_channels += out_channels
